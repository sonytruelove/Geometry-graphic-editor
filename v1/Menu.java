...
public class Menu {
   ...
   double dy=Double.parseDouble(y);
     ...
     Shape shape= new Circle(Double.parseDouble(x),Double.parseDouble(y),Double.parseDouble(r));
       Field.add(shape);
...
if(Math.abs((int)dx)>GUI.getMaxSize()/2||Math.abs((int)dy)>GUI.getMaxSize()/2) return "Выход за пределы поля";
    Field.add(new Rectangle(Double.parseDouble(x),Double.parseDouble(y),Double.parseDouble(a),Double.parseDouble(b)));
    return("Объект создан");
  }
...
return("Не числовое значение,уберите лишние знаки(+,- и т.д)");
  ...
  public static String deleteObject(String id){
    ...
    return("<html>Объект удален: <br>"+sh.output()+"</html>");
  }
